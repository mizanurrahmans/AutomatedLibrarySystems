...
<link href="./css/vstylesheet.css" rel="stylesheet" type="text/css" />
	...
	<div id="banner">
		...
		<br />
		<br />
		<span class="SubHead">Total Students</span>
		<br />
		<br />
		...
		<table id="customers">
			...
			if($result->num_rows >0){
				while($row = $result-> fetch_assoc()) {
					echo "<tr><td>" . $row["sid"] . "</td><td>" . $row["name"] . "</td><td>" . $row["roll"] . "</td><td>" . $row["phone"] . "</td><td>" . $row["branch"] . "</td><td>" . $row["sem"] . "</td><td>" . $row["email"] . "</td><td>"; 					
				}
			}
			else {
				echo "No Results";
